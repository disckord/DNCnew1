...
#define SHADOW_RES 4096.0

/* SHADOWRES:4096 */				//affect shadow map resolution (change SHADOW_RES with it!)
...
float torch_lightmap = pow(aux.b,7);
...
if (land > 0.9 ) {
vec3 torchcolor = vec3(1.0,1,1);
if (false){
vec3 sunlight_color =  vec3((sunlight_color.r + sunlight_color.b + sunlight_color.g)/3,(sunlight_color.r + sunlight_color.b + sunlight_color.g)/3,(sunlight_color.r + sunlight_color.b + sunlight_color.g)/3);

vec3 ambient_color =  vec3((ambient_color.r + ambient_color.b + ambient_color.g)/3,(ambient_color.r + ambient_color.b + ambient_color.g)/3,(ambient_color.r + ambient_color.b + ambient_color.g)/3);
}
...
//color_sunlight *= mix(1.0,1.0,1.0);
