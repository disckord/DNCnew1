...
public class Chunk
{
    ...
    private boolean isGapLightingNotUpdated;
    ...
    public Chunk(World par1World, int par2, int par3)
    {
        ...
        this.isGapLightingNotUpdated = false;
    ...
    public void generateSkylightMap()
    {
        ...
        for (var2 = 0; var2 < 16; ++var2)
        {
            ...
            while (var3 < 16)
            {
                this.precipitationHeightMap[var2 + (var3 * 16)] = -999;
                int var4 = var1 + 16 - 1;
                ...
                while (true)
                {
                    if (var4 > 0)
                    {
                    	//this part makes shade under tree / transparent blocks
                        ...
                        this.heightMap[var3 * 16 + var2] = var4;
                    ...
                    if (!this.worldObj.provider.hasNoSky)
                    {
                        ...
                        do
                        {
                            ...
                            if (var4 > 0)
                            {
                                ExtendedBlockStorage var6 = this.storageArrays[var5 / 16];

                                if (var6 != null)
                                {
                                    var6.setExtSkylightValue(var2, var5 & 15, var3, var4);
                                    this.worldObj.markBlockForRenderUpdate((this.xPosition * 16) + var2, var5, (this.zPosition * 16) + var3);
                                }
                            }
    ...
    private void propagateSkylightOcclusion(int par1, int par2)
    {
        ...
        this.isGapLightingNotUpdated = true;
    }
            ...
            for (int var1 = 0; var1 < 16; ++var1)
            {
                for (int var2 = 0; var2 < 16; ++var2)
                {
                    if (this.updateSkylightColumns[var1 + var2 * 16])
                    {
                        ...
                        int y = this.getHeightValue(var1, var2);
                        int x = this.xPosition * 16 + var1;
                        int z = this.zPosition * 16 + var2;
                        int var6 = this.worldObj.getChunkHeightMapMinimum(x - 1, z);
                        int var7 = this.worldObj.getChunkHeightMapMinimum(x + 1, z);
                        int var8 = this.worldObj.getChunkHeightMapMinimum(x, z - 1);
                        int var9 = this.worldObj.getChunkHeightMapMinimum(x, z + 1);
                        ...
                        this.checkSkylightNeighborHeight(x, z, var6);
                        this.checkSkylightNeighborHeight(x - 1, z, y);
                        this.checkSkylightNeighborHeight(x + 1, z, y);
                        this.checkSkylightNeighborHeight(x, z - 1, y);
                        this.checkSkylightNeighborHeight(x, z + 1, y);
                    }
                }
            }
    ...
    {
        int currentHeight = this.worldObj.getHeightValue(x, z);

        if (currentHeight > y)
        {
            this.updateSkylightNeighborHeight(x, z, y, currentHeight + 1);
        }
        else if (currentHeight < y)
        {
            this.updateSkylightNeighborHeight(x, z, currentHeight, y + 1);
        }
    }

    private void updateSkylightNeighborHeight(int x, int z, int y, int range)
    {
        if (range > y && this.worldObj.doChunksNearChunkExist(x, 0, z, 16))
        {
            for (int yitterated = y; yitterated < range; ++yitterated)
            {
                this.worldObj.updateLightByType(EnumSkyBlock.Sky, x, yitterated, z);
            }
    ...
    {
        if (this.isGapLightingNotUpdated && !this.worldObj.provider.hasNoSky)
        {
