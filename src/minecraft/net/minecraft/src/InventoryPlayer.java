...
public class InventoryPlayer implements IInventory
{
    ...
    //set to true whenever the player has armor that causes health regen.
    public boolean hasRegenArmor = false;
    public boolean hasSpeedArmor = false;
    public boolean hasBrightHelmet = false;
    
    public boolean getHasBrightHelmet()
    {
    	if(
    			this.armorInventory[3] != null
    			&& this.armorInventory[3].getItem() == Item.helmetHARDGOLD
    			&& this.armorInventory[1] != null 
    			&& this.armorInventory[1].getItem() == Item.legsHARDGOLD
    			&& this.armorInventory[2] != null 
    			&& this.armorInventory[2].getItem() == Item.plateHARDGOLD
    			&& this.armorInventory[0] !=null
    			&& this.armorInventory[0].getItem() == Item.bootsHARDGOLD
    			)
    	{
    		this.hasBrightHelmet = true;
    		this.hasRegenArmor = true;
    		this.hasSpeedArmor = true;
    	}
    	else
    	{
    		this.hasRegenArmor = false;
    		this.hasSpeedArmor = false;
    		this.hasBrightHelmet = false;
    	}
    	return this.hasBrightHelmet;
    }
    
    public boolean getHasSpeedArmor()
    {
    	/* if (this.armorInventory[1] != null && this.armorInventory[1].getItem() == Item.legsHARDGOLD)
         {
            this.hasSpeedArmor = true;
         }
         else
 	...
 	return this.hasSpeedArmor;
    }
    
    public boolean getHasRegenArmor()
    {/*
            if (this.armorInventory[2] != null && this.armorInventory[2].getItem() == Item.plateHARDGOLD)
            {
               this.hasRegenArmor = true;
            }
            else
