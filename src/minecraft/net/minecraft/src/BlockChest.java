...
public class BlockChest extends BlockContainer
{
    ...
    Icon[] iconArray;
	String[] sides = new String[]{"chest_0", "chest_1", "chest_2", "chest_3", "chest_4", "chest_5","chest_6"};
    ...
    public Icon getBlockTexture(IBlockAccess iblockaccess, int x, int y, int z, int metadata)
    {
        if(metadata == 1)
        {
            return iconArray[0];
        ...
        int blockAtNegZ = iblockaccess.getBlockId(x, y, z - 1);
        int blockAtPosZ = iblockaccess.getBlockId(x, y, z + 1);
        int blockAtNegX = iblockaccess.getBlockId(x - 1, y, z);
        int blockAtPosX = iblockaccess.getBlockId(x + 1, y, z);
        if(blockAtNegZ == blockID || blockAtPosZ == blockID)
        {
            ...
            int state = 0;
            if(blockAtNegZ == blockID)
            {
                state = -1;
            }
            int k2 = iblockaccess.getBlockId(x - 1, y, blockAtNegZ != blockID ? z + 1 : z - 1);
            int i3 = iblockaccess.getBlockId(x + 1, y, blockAtNegZ != blockID ? z + 1 : z - 1);
            if(metadata == 4)
            {
                state = -1 - state;
            }
            byte byte1 = 5;
            if((Block.opaqueCubeLookup[blockAtNegX] || Block.opaqueCubeLookup[k2]) && !Block.opaqueCubeLookup[blockAtPosX] && !Block.opaqueCubeLookup[i3])
            {
                byte1 = 5;
            }
            if((Block.opaqueCubeLookup[blockAtPosX] || Block.opaqueCubeLookup[i3]) && !Block.opaqueCubeLookup[blockAtNegX] && !Block.opaqueCubeLookup[k2])
            {
                byte1 = 4;
            }
            // +16 - 1 is left side, +16 is right side (locked face of double chest)
            // +32 - 1 is left side, +32 is right side (back face of double chest)
           // return (metadata != byte1 ? blockIndexInTexture + 32 : blockIndexInTexture + 16) + state;
            if(metadata != byte1)
            {
            	if(state == -1)
            	{
        ...
        if(blockAtNegX == blockID || blockAtPosX == blockID)
        {
            ...
            int state2 = 0;
            if(blockAtNegX == blockID)
            {
                state2 = -1;
            }
            int l2 = iblockaccess.getBlockId(blockAtNegX != blockID ? x + 1 : x - 1, y, z - 1);
            int j3 = iblockaccess.getBlockId(blockAtNegX != blockID ? x + 1 : x - 1, y, z + 1);
            if(metadata == 3)
            {
                state2 = -1 - state2;
            }
            byte byte2 = 3;
            if((Block.opaqueCubeLookup[blockAtNegZ] || Block.opaqueCubeLookup[l2]) && !Block.opaqueCubeLookup[blockAtPosZ] && !Block.opaqueCubeLookup[j3])
            {
                byte2 = 3;
            }
            if((Block.opaqueCubeLookup[blockAtPosZ] || Block.opaqueCubeLookup[j3]) && !Block.opaqueCubeLookup[blockAtNegZ] && !Block.opaqueCubeLookup[l2])
            {
                byte2 = 2;
            }
          //  return (metadata != byte2 ? blockIndexInTexture + 32 : blockIndexInTexture + 16) + state2;
            if(metadata != byte2)
            {
            	if(state2 == -1)
            	{
        ...
        byte byte0 = 3;
        if(Block.opaqueCubeLookup[blockAtNegZ] && !Block.opaqueCubeLookup[blockAtPosZ])
        {
            byte0 = 3;
        }
        if(Block.opaqueCubeLookup[blockAtPosZ] && !Block.opaqueCubeLookup[blockAtNegZ])
        {
            byte0 = 2;
        }
        if(Block.opaqueCubeLookup[blockAtNegX] && !Block.opaqueCubeLookup[blockAtPosX])
        {
            byte0 = 5;
        }
        if(Block.opaqueCubeLookup[blockAtPosX] && !Block.opaqueCubeLookup[blockAtNegX])
        {
            byte0 = 4;
        }
        return metadata != byte0 ? iconArray[1] : iconArray[2];
    }
    /**
    ...
    public void registerIcons(IconRegister par1IconRegister)
    {
        ...
        iconArray = new Icon[sides.length];
        for(int i = 0; i < sides.length; i++)
        {
        	iconArray[i] = par1IconRegister.registerIcon(sides[i]);
        }
        this.blockIcon = par1IconRegister.registerIcon(sides[1]);
    }
}
