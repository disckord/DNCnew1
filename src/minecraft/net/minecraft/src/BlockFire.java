...
public class BlockFire extends Block
{
    ...
    {this.setBurnRate(Block.planks.blockID, 800, 20);
    this.setBurnRate(Block.woodDoubleSlab.blockID, 800, 20);
    this.setBurnRate(Block.woodSingleSlab.blockID, 800, 20);
    this.setBurnRate(Block.fence.blockID, 800, 20);
    this.setBurnRate(Block.stairsWoodOak.blockID, 800, 20);
    this.setBurnRate(Block.stairsWoodBirch.blockID, 800, 20);
    this.setBurnRate(Block.stairsWoodSpruce.blockID, 800, 20);
    this.setBurnRate(Block.stairsWoodJungle.blockID, 800, 20);
    this.setBurnRate(Block.wood.blockID, 800, 5);
    this.setBurnRate(Block.leaves.blockID, 900, 60);
    this.setBurnRate(Block.bookShelf.blockID, 800, 20);
    this.setBurnRate(Block.tnt.blockID, 800, 100);
    this.setBurnRate(Block.tallGrass.blockID, 900, 100);
    this.setBurnRate(Block.cloth.blockID, 900, 60);
    this.setBurnRate(Block.vine.blockID, 900, 100);
    this.setBurnRate(Block.coalBlock.blockID, 800, 2);
    }
    ...
    public void updateTick(World par1World, int x, int y, int z, Random par5Random)
    {
        ...
        {
            boolean var6 = par1World.getBlockId(x, y - 1, z) == Block.netherrack.blockID;

            if (par1World.provider instanceof WorldProviderEnd && par1World.getBlockId(x, y - 1, z) == Block.bedrock.blockID)
            {
                var6 = true;
            }
            ...
            if (!var6 && par1World.isRaining() && (par1World.canLightningStrikeAt(x, y, z) || par1World.canLightningStrikeAt(x - 1, y, z) || par1World.canLightningStrikeAt(x + 1, y, z) || par1World.canLightningStrikeAt(x, y, z - 1) || par1World.canLightningStrikeAt(x, y, z + 1)))
            {
            ...
            {
                int var7 = par1World.getBlockMetadata(x, y, z);

                if (var7 < 15)
                {
                    par1World.setBlockMetadataWithNotify(x, y, z, var7 + par5Random.nextInt(3) / 2, 4);
                }

                par1World.scheduleBlockUpdate(x, y, z, this.blockID, this.tickRate(par1World) + par5Random.nextInt(10));

                if (!var6 && !this.canNeighborBurn(par1World, x, y, z))
                {
                    if (!par1World.doesBlockHaveSolidTopSurface(x, y - 1, z) || var7 > 3)
                    {
                        par1World.setBlockToAir(x, y, z);
                    }
                }
                else if (!var6 && !this.canBlockCatchFire(par1World, x, y - 1, z) && var7 == 15 && par5Random.nextInt(4) == 0)
                {
                    par1World.setBlockToAir(x, y, z);
                }
                else
                {
                    boolean var8 = par1World.isBlockHighHumidity(x, y, z);
                    byte var9 = 0;
                    ...
                    this.tryToCatchBlockOnFire(par1World, x + 1, y, z, 300 + var9, par5Random, var7);
                    this.tryToCatchBlockOnFire(par1World, x - 1, y, z, 300 + var9, par5Random, var7);
                    this.tryToCatchBlockOnFire(par1World, x, y - 1, z, 250 + var9, par5Random, var7);
                    this.tryToCatchBlockOnFire(par1World, x, y + 1, z, 250 + var9, par5Random, var7);
                    this.tryToCatchBlockOnFire(par1World, x, y, z - 1, 300 + var9, par5Random, var7);
                    this.tryToCatchBlockOnFire(par1World, x, y, z + 1, 300 + var9, par5Random, var7);
                            ...
                            for (int var12 = y - 1; var12 <= y + 4; ++var12)
                            {
                                if (var10 != x || var12 != y || var11 != z)
                                {
                                    ...
                                    if (var14 > 0)
                                    {
                                        ...
                                        if (var15 > 0 && par5Random.nextInt(var13) <= var15 && (!par1World.isRaining() || !par1World.canLightningStrikeAt(var10, var12, var11)) && !par1World.canLightningStrikeAt(var10 - 1, var12, z) && !par1World.canLightningStrikeAt(var10 + 1, var12, var11) && !par1World.canLightningStrikeAt(var10, var12, var11 - 1) && !par1World.canLightningStrikeAt(var10, var12, var11 + 1))
                                        {
    ...
    public void randomDisplayTick(World par1World, int x, int y, int z, Random par5Random)
    {
        ...
        {
            par1World.playSound((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F), "fire.fire", 1.0F + par5Random.nextFloat(), par5Random.nextFloat() * 0.7F + 0.3F, false);
        }
        if (par5Random.nextInt(40) == 0 && par1World.isRaining() && par1World.canLightningStrikeAt(x, y, z))
        {
            par1World.playSound((double)((float)x + 0.5F), (double)((float)y + 0.5F), (double)((float)z + 0.5F), "random.fizz", 0.5F , 2.6F + (par1World.rand.nextFloat() - par1World.rand.nextFloat() ) * 0.8F, false);
        }
        ...
        if (!par1World.doesBlockHaveSolidTopSurface(x, y - 1, z) && !Block.fire.canBlockCatchFire(par1World, x, y - 1, z))
        {
            if (Block.fire.canBlockCatchFire(par1World, x - 1, y, z))
            {
                for (var6 = 0; var6 < 2; ++var6)
                {
                    var7 = (float)x + par5Random.nextFloat() * 0.1F;
                    var8 = (float)y + par5Random.nextFloat();
                    var9 = (float)z + par5Random.nextFloat();
            ...
            if (Block.fire.canBlockCatchFire(par1World, x + 1, y, z))
            {
                for (var6 = 0; var6 < 2; ++var6)
                {
                    var7 = (float)(x + 1) - par5Random.nextFloat() * 0.1F;
                    var8 = (float)y + par5Random.nextFloat();
                    var9 = (float)z + par5Random.nextFloat();
                    par1World.spawnParticle("largesmoke", (double)var7, (double)var8, (double)var9, 0.0D, 0.0D, 0.0D);
                }
            ...
            if (Block.fire.canBlockCatchFire(par1World, x, y, z - 1))
            {
                for (var6 = 0; var6 < 2; ++var6)
                {
                    var7 = (float)x + par5Random.nextFloat();
                    var8 = (float)y + par5Random.nextFloat();
                    var9 = (float)z + par5Random.nextFloat() * 0.1F;
                    par1World.spawnParticle("largesmoke", (double)var7, (double)var8, (double)var9, 0.0D, 0.0D, 0.0D);
                }
            }
            ...
            if (Block.fire.canBlockCatchFire(par1World, x, y, z + 1))
            {
                for (var6 = 0; var6 < 2; ++var6)
                {
                    var7 = (float)x + par5Random.nextFloat();
                    var8 = (float)y + par5Random.nextFloat();
                    var9 = (float)(z + 1) - par5Random.nextFloat() * 0.1F;
                    par1World.spawnParticle("largesmoke", (double)var7, (double)var8, (double)var9, 0.0D, 0.0D, 0.0D);
                }
            }

            if (Block.fire.canBlockCatchFire(par1World, x, y + 1, z))
            {
                for (var6 = 0; var6 < 2; ++var6)
                {
                    var7 = (float)x + par5Random.nextFloat();
                    var8 = (float)(y + 1) - par5Random.nextFloat() * 0.1F;
                    var9 = (float)z + par5Random.nextFloat();
                    par1World.spawnParticle("largesmoke", (double)var7, (double)var8, (double)var9, 0.0D, 0.0D, 0.0D);
                }
            }
        ...
        else
        {
            ...
            {
                var7 = (float)x + par5Random.nextFloat();
                var8 = (float)y + par5Random.nextFloat() * 0.5F + 0.5F;
                var9 = (float)z + par5Random.nextFloat();
                par1World.spawnParticle("largesmoke", (double)var7, (double)var8, (double)var9, 0.0D, 0.0D, 0.0D);
            }
        }
